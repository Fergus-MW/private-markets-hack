...
Cloud Run uses service-account credentials for Vertex (`GOOGLE_CLOUD_PROJECT`, `GEMINI_MODEL`). `GEMINI_API_KEY` remains an optional local development fallback. No model key is required in production. Every model-backed role is configured for `gemini-3.1-pro-preview`; deterministic checkers, ingestion control and link generation do not invoke a model.
