...
* Explain: reads back what earlier runs recorded — run history, `finding` nodes and deterministic `check_result` nodes — and explains them. It materializes nothing, runs no checker and produces no deliverable.
...
Cloud Run uses service-account credentials for Vertex (`GOOGLE_CLOUD_PROJECT`, `GEMINI_MODEL`). `GEMINI_API_KEY` remains an optional local development fallback. No model key is required in production. The configured model is `gemini-3.5-flash`, verified with live synthetic tool-calling and cited-rule-generation requests.
