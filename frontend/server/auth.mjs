...
export function configuration(env = process.env) {
  ...
  const accounts = JSON.parse(env.GOOGLE_CONNECTOR_ACCOUNTS || '{}');
  const usedSecrets = new Set();
  for (const [email, targets] of Object.entries(accounts)) {
    if (email !== email.toLowerCase() || !email.includes('@')) throw new Error('Account keys must be lowercase email addresses');
    for (const provider of ['gmail', 'drive']) {
      const secret = targets[provider];
      if (!/^projects\/[a-z0-9-]+\/secrets\/connector-[a-z0-9-]+-oauth$/.test(secret || '') || usedSecrets.has(secret)) throw new Error('Each account needs distinct Gmail and Drive connector secrets');
  ...
  return {
    origin: origin.origin, secure: origin.protocol === 'https:', accounts, key,
    clientId: env.GOOGLE_OAUTH_CLIENT_ID, clientSecret: env.GOOGLE_OAUTH_CLIENT_SECRET,
    enabled: Boolean(env.GOOGLE_OAUTH_CLIENT_ID && env.GOOGLE_OAUTH_CLIENT_SECRET && key && Object.keys(accounts).length),
  };
}
...
export function createAuth(config, dependencies = {}) {
  ...
  const saveSecret = dependencies.saveSecret || (async (name, credentials) => {
    const client = await cloud.getClient();
    await client.request({ url: `https://secretmanager.googleapis.com/v1/${name}:addVersion`, method: 'POST',
      data: { payload: { data: Buffer.from(JSON.stringify(credentials)).toString('base64') } } });
  });
  ...
  return async function auth(req, res, url) {
    if (url.pathname === '/api/session') {
      ...
      res.end(JSON.stringify(session?.kind === 'connection'
        ? { connected: true, email: session.email, configured: config.enabled }
    ...
    try {
      ...
      const targets = config.accounts[identity.email?.toLowerCase()];
      if (!targets) throw new ConnectionError('account');
      ...
      // Both must persist before the UI reports a successful connection.
      await saveSecret(targets.gmail, credentials);
      await saveSecret(targets.drive, credentials);
      const session = seal({ kind: 'connection', email: identity.email, expires: Date.now() + 3_600_000 }, config.key);
